...
class RHAPIRaceProviderAdapter(RaceProviderPort):
    """RaceProvider backed by the injected RaceHostPort implementation."""

    def __init__(self, race_host):
        self._race_host = race_host

    def get_current_heat(self) -> int | None:
        race = getattr(self._race_host, "race", None)
        return getattr(race, "current_heat", None)

    def get_pilot_assignments(self) -> list[tuple[int, str]]:
        ctx = self._race_host.racecontext
        if not ctx:
            return []
    ...
    def on_race_start(self, handler: Callable[[object], None]) -> None:
        self._race_host.on(Evt.RACE_START, handler)

    def on_race_finish(self, handler: Callable[[object], None]) -> None:
        self._race_host.on(Evt.RACE_FINISH, handler)

    def on_race_stop(self, handler: Callable[[object], None]) -> None:
        self._race_host.on(Evt.RACE_STOP, handler)
